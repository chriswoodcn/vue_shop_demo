...
;(function (window, document, Math) {
  ...
  function IScroll(el, options) {
    ...
    this.options.bounceEasing =
      typeof this.options.bounceEasing == 'string' ? utils.ease[this.options.bounceEasing] || utils.ease.circular : this.options.bounceEasing
  ...
  IScroll.prototype = {
    ...
    goToPage: function (x, y, time, easing) {
      ...
      time =
        time === undefined
          ? this.options.snapSpeed || Math.max(Math.max(Math.min(Math.abs(posX - this.x), 1000), Math.min(Math.abs(posY - this.y), 1000)), 300)
          : time
